...
- **Mode B (draft text):** write the draft to a temporary file at
  `sandbox/.tmp_latex_verify_<unix_timestamp>.md`. (The `sandbox/` tree is
  git-ignored, so temp files are safe.) Delete the temp file at the end
  of the turn, even on error.
