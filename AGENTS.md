...
- **`status` vocabulary** is the linear per-paper pipeline plus `tutored`, with `compared` for multi-paper artifacts. Use the value that matches the artifact's role (e.g. `spec.md` → `dissected`, `code_map.md` → `implemented`, `critic_reviews.md` / `code_review.md` → `critiqued` (the audit's source determines which file gets written; see Learning suite "critic" entry), tutor/explainer files → `tutored`, `comparison.md` → `compared`).
...
- Math notation: use LaTeX between `$ ... $` for inline math and `$$ ... $$` for display math.
  ...
  **Exception — chat/console output.** When an agent is responding directly in the Cursor chat console (not writing a vault or repo file), it may use Unicode math characters (`θ`, `α`, `Σ`, `μ`, `∇`, ...) for inline symbols, because Cursor's chat does not always render inline `$...$`. Display math in chat should still use `$$ ... $$` (which Cursor renders reliably). The "no Unicode math" rule continues to apply to all written files (vault markdown, repo code/comments, logs).
...
- `critic_reviews.md` — audit from the `critic` subagent (when `code_map.md` source is `official`).
- `code_review.md` — fidelity audit from the `critic` subagent (when `code_map.md` source is `reconstructed`; sibling of `critic_reviews.md`, same schema, distinct file so a paper audited under both sources keeps both audits).
...
- `critic` audits claims, reproducibility, and paper-code alignment. The audit adapts to the `code_map.md` source (`ml-critique` "Audit source") and writes to a different file for each source (siblings, never one overwriting the other): for `official` code it weighs author choices and upstream/dataset reproducibility, written to `critic_reviews.md`; for `reconstructed` code it becomes a **fidelity audit** (does the coder's reconstruction drift from the paper's math?) with reconstruction-fidelity reproducibility rows — the firewalled **hop-2-vs-spec** check — written to `code_review.md`. The two-file split (changed 2026-06-16, see [`log/2026-06-16-critic-code-review-and-coder-extension.md`](./log/2026-06-16-critic-code-review-and-coder-extension.md)) makes regenerate-prompt compliance trivial (each source's audit is a first-time write to its own file) and lets a paper audited under both sources keep both audits. A backend **blueprint-check mode** (invoked by the `implementer`, never the user) audits a draft `code_blueprint.md` **pre-emission** against the critic's own independent reading of the paper's math, returning a PASS/FAIL verdict without writing a file (the **hop-1** guard). A backend **extraction-fidelity mode** (invoked by the `experimenter`, never the user) gates Stage-2 component surgery (multi-method) **pre-run**: Check A audits each `extracted.py` against its `code_map.md` source (per-paper hard gate), Check B audits the synthesized `scaffold.py` against the shared principle; PASS/FAIL, no file. A backend **extension-fidelity mode** (added 2026-06-16, invoked by the `experimenter`, never the user) gates Stage-2 extension regime (single-method) **pre-run**: audits each `methods/<slug>/extended.py` and `run.py` against the audited Stage-1 `method.py` / `code_map.md` for unauthorized base-method rewrites, scope drift, or wiring drift; PASS/FAIL, no file (no scaffold, so no Check B). All four gate modes are the firewalled generator/discriminator check from the two-memory design.
...
- `experimenter` (**converted to skill + command 2026-06-15**; **Build-evaluate sub-phase + filesystem-state-aware topic detection (B+A protocol) shipped 2026-06-17**; conversational rewrite shipped 2026-06-09; design phase shipped 2026-06-02; Stage-2 hand-off wired 2026-06-04) is the user-facing **pair-designer** for empirical experiments. **Loaded as a skill** at `.cursor/skills/experimenter/SKILL.md` via the `/experimenter` command, so the main chat agent runs it directly (no subagent relay). Two-phase protocol: **Plan phase** (default) is open prose dialogue with no `AskQuestion` / multiple-choice menus and no presumption of research type (methods comparison, ablation, reproduction, sensitivity, exploration, or custom — all emerge from the conversation); **Build phase** (user-triggered) splits into **Build-implement** (write `design.md`, invoke `coder` Stage 2 — **component surgery** for ≥ 2 members with the §5.2 seam contract gated by extraction-fidelity + Seam-B user-check, or **extension regime** for exactly 1 member with the §5.2-variant extension scope gated by extension-fidelity, added 2026-06-16) and **Build-evaluate** (invoke `evaluator` on populated `run/results/`, relay `findings.md` summary; shipped 2026-06-17). On topic entry, **filesystem-state detection** routes to Plan / Plan-resume / Build-evaluate based on whether `design.md` and `run/results/*.json` exist — the **B+A protocol**: short runs stay in one chat (the user signals "done", the experimenter transitions to Build-evaluate), long runs span days (the user closes the chat after Build-implement, re-opens `/experimenter <topic>` later, state-detection picks up at evaluation). The skill sketches the `design.md` section list explicitly to the user before writing, and the Plan→Build switch requires an explicit user signal + chat plan summary + user confirmation. Owns the experiment design and data-synthesis *decisions* including the **§5.2 comparison seam** (conditional on research type, load-bearing for Stage-2 coding). **Pause discipline:** never invokes the `evaluator` on empty / smoke results; emits a "run X, ping me / re-open later" instruction and ends the turn. Writes `design.md` / `findings.md` to `<vault>/experiments/<topic>/` (the latter via the evaluator). Writes no code itself. Invoked via `/experimenter <topic>`.
- `coder` is the only agent that writes **runnable code**, split into two stages (design: [`log/2026-06-03-two-stage-coder-design.md`](./log/2026-06-03-two-stage-coder-design.md)). It **spans both suites**: Stage 1 is a per-paper, user-invokable step that belongs with the Learning suite's output (it writes to `<vault>/<slug>/code/`), while Stage 2 is the backend mode the Experimenter suite drives. Stage 2 has **two regimes** (`ml-experiment-code` § Stage 2): **component surgery** for multi-method experiments (≥ 2 papers; synthesizes a shared scaffold, extracts each member's divergent component into `methods/<slug>/extracted.py`; gated by extraction-fidelity) and **extension regime** for single-method experiments (exactly 1 paper; inherits/composes the audited Stage-1 `method.py` into `methods/<slug>/extended.py`; gated by extension-fidelity). Extension regime was added 2026-06-16 ([`log/2026-06-16-critic-code-review-and-coder-extension.md`](./log/2026-06-16-critic-code-review-and-coder-extension.md)) so single-method experiments (ablations, sensitivity sweeps, planted-signal probes) stop being shoehorned into a 1-method "comparison" with no comparator. It is documented here because its two stages are one agent. **Stage 1 (shipped 2026-06-04, user-invokable)** writes reusable, invariant-validated method code for one paper to `vault_code_dir(slug)` (`<vault>/<slug>/code/`: `method.py` + `test_invariants.py`), from the paper's `code_blueprint.md` (primary route, no official code) or a reimplementation of its mapped upstream code. It exposes a hybrid `Method` interface (paper-natural guts + one documented entry point + an I/O contract block) and runs the blueprint's §4 invariants as runtime assertions on synthetic input before declaring done — the **hop-2-vs-blueprint guard**. It does **not** write the algorithm↔code walkthrough: after Stage 1, the `implementer` maps `method.py` into `code_map.md` (the `reconstructed` source) and the `critic` audits it against the spec (hop-2-vs-spec) — keeping documentation/audit off the code's author. Invoke with `/coder code <slug>`. **Stage 2 (component surgery, shipped 2026-06-04, backend-only)** is **not** black-box wrapping: from the experiment's `design.md` §5.2 seam it synthesizes a shared scaffold (principle + task fixed, pluggable slot `Protocol`) and extracts each paper's divergent component into `repo_experiments_dir(topic)/methods/<slug>/extracted.py` via the borrow ladder (import-direct / extract-and-refactor), preserving the source computation. Gated by the `critic`'s extraction-fidelity audit (Check A: each `extracted.py` vs. `code_map.md`, per-paper hard gate; Check B: `scaffold.py` vs. the shared principle) plus opportunistic coder-run behavioral equivalence. Invoked by the `experimenter`, not the user. The kickoff wrapping framing ([`log/2026-06-04-stage2-coder-adapt-kickoff.md`](./log/2026-06-04-stage2-coder-adapt-kickoff.md)) is superseded by the component-surgery redesign ([`log/2026-06-04-stage2-regime2-component-surgery-design.md`](./log/2026-06-04-stage2-regime2-component-surgery-design.md)).
- `evaluator` (**shipped 2026-06-17**) is **backend-only**, invoked by the `experimenter` during the **Build-evaluate** sub-phase, never by the user. Reads `design.md` (hypotheses, criterion, metrics) and `repo_experiments_dir(topic)/run/results/*.json`; writes `findings.md` to `vault_experiments_dir(topic)/`. Returns the path of the written file plus a one-paragraph summary — **no PASS/FAIL**; the user reads `findings.md` and judges the design as a whole. Honesty discipline `[A]` paper-anchored / `[B]` reader-inferred / `[E]` empirically grounded by *this* run is mandatory in every section past the header. On under-spec runs (smoke output, missing seeds, missing metric, errored trajectory) the evaluator stays permissive and tags affected hypotheses `[INSUFFICIENT-RUN]` with ledger status `inconclusive` — it does not refuse. Refusal is the experimenter's job (pause discipline, never invoke on empty results). Skill: `ml-evaluation`. See [`log/2026-06-17-evaluator-build.md`](./log/2026-06-17-evaluator-build.md).
...
- `experimenter` → `.cursor/skills/experimenter/SKILL.md` (behavior, loaded by `/experimenter` command, **shipped 2026-06-15**); reads `.cursor/skills/ml-experiment-design/SKILL.md` (schema, **shipped 2026-06-02**) only when entering Build phase.
