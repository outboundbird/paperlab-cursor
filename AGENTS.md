...
- `experimenter` (**conversational rewrite shipped 2026-06-09**; design phase shipped 2026-06-02; Stage-2 hand-off wired 2026-06-04) is the user-facing **pair-designer** for empirical experiments. Two-phase protocol: **Plan phase** (default) is open prose dialogue with no `AskQuestion` / multiple-choice menus and no presumption of research type (methods comparison, ablation, reproduction, sensitivity, exploration, or custom — all emerge from the conversation); **Build phase** (user-triggered) writes `design.md`, invokes `coder` Stage 2 with the §5.2 seam contract, routes the critic extraction-fidelity gate + Seam-B user-check, and runs to **results emitted**. The agent sketches the `design.md` section list explicitly to the user before writing, and the Plan→Build switch requires an explicit user signal + chat plan summary + user confirmation. Owns the experiment design and data-synthesis *decisions* including the **§5.2 comparison seam** (conditional on research type, load-bearing for Stage-2 coding). Writes `design.md` / `findings.md` to `<vault>/experiments/<topic>/`. It writes no code itself; the full implement/run orchestration protocol is still being fleshed out, and `findings.md` awaits the `evaluator`. Invoked via `/experimenter <topic>`.
