...
- `experimenter` (**converted to skill + command 2026-06-15**; conversational rewrite shipped 2026-06-09; design phase shipped 2026-06-02; Stage-2 hand-off wired 2026-06-04) is the user-facing **pair-designer** for empirical experiments. **Loaded as a skill** at `.cursor/skills/experimenter/SKILL.md` via the `/experimenter` command, so the main chat agent runs it directly (no subagent relay). Two-phase protocol: **Plan phase** (default) is open prose dialogue with no `AskQuestion` / multiple-choice menus and no presumption of research type (methods comparison, ablation, reproduction, sensitivity, exploration, or custom — all emerge from the conversation); **Build phase** (user-triggered) writes `design.md`, invokes `coder` Stage 2 — **component surgery** (≥ 2 members, with the §5.2 seam contract, gated by extraction-fidelity + Seam-B user-check) or **extension regime** (exactly 1 member, with the §5.2-variant extension scope, gated by extension-fidelity; added 2026-06-16) — and runs to **results emitted**. The skill sketches the `design.md` section list explicitly to the user before writing, and the Plan→Build switch requires an explicit user signal + chat plan summary + user confirmation. Owns the experiment design and data-synthesis *decisions* including the **§5.2 comparison seam** (conditional on research type, load-bearing for Stage-2 coding). Writes `design.md` / `findings.md` to `<vault>/experiments/<topic>/`. It writes no code itself; the full implement/run orchestration protocol is still being fleshed out, and `findings.md` awaits the `evaluator`. Invoked via `/experimenter <topic>`.
