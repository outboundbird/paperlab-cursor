...
- `critic` audits claims, reproducibility, and paper-code alignment, then writes `critic_reviews.md` to the vault. A backend **blueprint-check mode** (invoked by the `implementer`, never the user) audits a draft `code_blueprint.md` **pre-emission** against the critic's own independent reading of the paper's math, returning a PASS/FAIL verdict without writing a file. This is the hop-1 guard for blueprints and the firewalled generator/discriminator check from the two-memory design.
