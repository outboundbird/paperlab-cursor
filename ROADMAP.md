...
- **⏭️ NEXT BUILD PRIORITY (P1) — Experimenter conversational rewrite.** Two `/experimenter` smoke runs (2026-06-05) showed the agent jumps straight to building on turn 1 — reads both specs, builds the comparison table, decides the seam, and pops a multiple-choice menu — without asking the user a single problem-setup question. **Incremental rule-patching failed twice** (R0/R0a/R0b, then a hardened R0a multiple-choice ban + R0c discuss-only). Diagnosis: the skill + agent are *structured like a production pipeline*, so rules read as soft preferences. **Resolution: structural rewrite** — make conversation the spine, demote the `design.md` schema to a reference appendix consulted only after the user says to write it up; first turn = greeting + one open question, then stop. Scope: experimenter only. Open: ban `AskQuestion` entirely vs. only until the user switches to building. Tracked in [`log/2026-06-05-experimenter-problem-framing-feedback.md`](./log/2026-06-05-experimenter-problem-framing-feedback.md) § "Follow-up" + GitHub issue. **Awaiting user decision on the rewrite shape before building.**
